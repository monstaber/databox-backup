...
import { buildTasks, makeSlug } from '../src/index.js';
...
describe('makeSlug', () => {
  it('strips Czech diacritics and lowercases', () => {
    assert.equal(
      makeSlug({ dmAnnotation: 'Rozhodnutí č. 42/2026' }),
      'rozhodnuti-c-42-2026',
    );
  });

  it('returns "untitled" for null or empty', () => {
    assert.equal(makeSlug({ dmAnnotation: null }), 'untitled');
    assert.equal(makeSlug({ dmAnnotation: '' }), 'untitled');
    assert.equal(makeSlug({ dmAnnotation: '///' }), 'untitled');
  });

  it('caps length at 80 chars', () => {
    const long = 'a'.repeat(200);
    assert.equal(makeSlug({ dmAnnotation: long }).length, 80);
  });

  it('collapses runs of non-word chars', () => {
    assert.equal(makeSlug({ dmAnnotation: 'Hello    world!!!' }), 'hello-world');
  });
});
